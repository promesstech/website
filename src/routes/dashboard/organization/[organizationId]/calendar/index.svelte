    ...
    {#each days as day, i}
        ...
        <div class="w-full flex flex-col h-full items-center pt-4 font-bold border border-solid border-opacity-10 border-secondary">
            ...
            <div class="flex flex-col overflow-y-scroll w-full">
                <div class="flex flex-col w-full mt-2 pr-4">
                    {#if i === 15}
                        <span class="flex items-center rounded-md bg-blue-4 text-xs font-bold px-2 h-6 truncate mb-2 hover:cursor-pointer">Product Release</span>
                    {/if}
                </div>
                <div class="flex flex-col pl-2">
                    {#if i === 19}
                        <div class="flex flex-row items-start w-fit hover:cursor-pointer">
                            ...
                            <span class="custom-scrollbar ml-1 font-bold text-xs w-fit truncate">This a reminder</span>
                        </div>
                    {/if}
                </div>
            </div>
